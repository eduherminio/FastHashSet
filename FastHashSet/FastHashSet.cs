...
namespace FastHashSet
{
    ...
    public class FastHashSet<T> : IReadOnlyCollection<T>, ISet<T>
    {
        ...
        private int _currentIndexIntoBucketsSizeArray;

        private int _bucketsModSize;

#if !Exclude_Check_For_Set_Modifications_In_Enumerator
        private int _incrementForEverySetModification;
#endif

        // resize the buckets array when the count reaches this value
        private int _resizeBucketsCountThreshold;

        private int _count;

        private int _nextBlankIndex;
        ...
        private int _firstBlankAtEndIndex;
        ...
        private void AddInitialUniqueValuesEnumerable(IEnumerable<T> collection)
        {
            ...
            if (IsHashing)
            {
                ...
                {
                    int hash = (_comparer.GetHashCode(item) & HighBitNotSet);
                    int hashIndex = hash % _bucketsModSize;

                    int index = _buckets[hashIndex];
                    _buckets[hashIndex] = _nextBlankIndex;

                    ref TNode t = ref _slots[_nextBlankIndex];
            ...
            else
            {
                foreach (T item in collection)
                {
                    _noHashArray[itemsCount++] = item;
                }
            }
#endif
            _count = itemsCount;
            _firstBlankAtEndIndex = _nextBlankIndex;
        }
        ...
        private void AddInitialEnumerableWithEnoughCapacity(IEnumerable<T> collection)
        {
            ...
            foreach (T item in collection)
            {
                ...
                for (int index = _buckets[hashIndex]; index != NullIndex;)
                {
                    ref TNode t = ref _slots[index];

                    if (t.hashOrNextIndexForBlanks == hash && _comparer.Equals(t.item, item))
                    {
                ...
                ref TNode tBlank = ref _slots[_nextBlankIndex];

                tBlank.hashOrNextIndexForBlanks = hash;
                tBlank.nextIndex = _buckets[hashIndex];
                tBlank.item = item;
                ...
                if (_count >= _resizeBucketsCountThreshold)
                {
        ...
        {
            if (collection is FastHashSet<T> fhset && Equals(fhset.Comparer, Comparer))
            {
                ...
                if (IsHashing)
                {
                    if (fhset.IsHashing)
                    {
                        ...
                        _nextBlankIndex = 1;
                        int maxNodeIndex = fhset._slots.Length - 1;
                        if (fhset._firstBlankAtEndIndex <= maxNodeIndex)
                        {
                            maxNodeIndex = fhset._firstBlankAtEndIndex - 1;
                        }

                        for (int i = 1; i <= maxNodeIndex; i++)
                        {
                            ref TNode t2 = ref fhset._slots[i];
                            if (t2.nextIndex != BlankNextIndexIndicator)
                            {
                                ...
                                t.hashOrNextIndexForBlanks = hash;
                                t.nextIndex = _buckets[hashIndex];
                                t.item = t2.item;
                    ...
                    else
                    {
                        ...
                        _nextBlankIndex = 1;
                        for (int i = 0; i < fhset._count; i++)
                        {
                            ref T item = ref _noHashArray[i];
            ...
            else
            {
                ...
                if (collection is HashSet<T> hset && Equals(hset.Comparer, Comparer))
                {
                    // a set with the same item comparer must have all items unique
                    // so Count will be the exact Count of the items added
                    // also don't have to check for equals of items
        ...
        private void SwitchToHashing(int capacityIncrease = -1)
        {
            ...
            if (_noHashArray != null)
            {
                ...
                _noHashArray = null; // this array can now be garbage collected because it is no longer referenced
            }
        ...
        private void InitHashing(int capacity = -1)
        {
            ...
            if (capacity == -1)
            {
                ...
                {
                    for (_currentIndexIntoBucketsSizeArray = 1; _currentIndexIntoBucketsSizeArray < BucketsSizeArray.Length; _currentIndexIntoBucketsSizeArray++)
                    {
                        newBucketsArraySize = BucketsSizeArray[_currentIndexIntoBucketsSizeArray];
                        if (newBucketsArraySize >= newSlotsArraySize)
                        {
                            break;
                        }
            ...
            _slots = new TNode[newSlotsArraySize]; // the slots array has an extra item as it's first item (0 index) that is for available items - the memory is wasted, but it simplifies things
            _buckets = new int[newBucketsArraySize]; // these will be initially set to 0, so make 0 the blank(available) value and reduce all indices by one to get to the actual index into the slots array
            _bucketsModSize = newBucketsArrayModSize;

            if (setThresh)
            {
                _resizeBucketsCountThreshold = (int)(newBucketsArrayModSize * LoadFactorConst);
            }
            else
            {
            ...
            _nextBlankIndex = 1; // start at 1 because 0 is the blank item
        ...
        {
            _noHashArray = new T[InitialArraySize];
        }
#endif
        ...
        private void CalcUsedItemsLoadFactorThreshold()
        {
            if (_buckets != null)
            {
                if (_buckets.Length == _bucketsModSize)
                {
                    _resizeBucketsCountThreshold = _slots.Length; // with this value, the buckets array should always resize after the slots array (in the same public function call)
                }
                else
                {
                    ...
                    _resizeBucketsCountThreshold = (int)(_bucketsModSize * LoadFactorConst);
                }
            }
        }
        ...
        public void CopyTo(T[] array, int arrayIndex, int count)
        {
            ...
            {
#endif
                int pastNodeIndex = _slots.Length;
                if (_firstBlankAtEndIndex < pastNodeIndex)
                {
                    pastNodeIndex = _firstBlankAtEndIndex;
                }

                int cnt = 0;
                for (int i = 1; i < pastNodeIndex; i++)
                {
                    if (_slots[i].nextIndex != BlankNextIndexIndicator)
                    {
                        array[arrayIndex++] = _slots[i].item;
                        if (++cnt == count)
                        {
                            break;
                        }
            ...
            else
            {
                ...
                for (int i = 0; i < cnt; i++)
                {
                    array[arrayIndex++] = _noHashArray[i];
                }
            }
#endif
        }
        ...
        public int ExcessCapacity
        {
            get
            {
                ...
                {
#endif
                    excessCapacity = _slots.Length - _firstBlankAtEndIndex;
#if !Exclude_No_Hash_Array_Implementation
                }
                else
                {
                    excessCapacity = _noHashArray.Length - _count;
                }
#endif
        ...
        public int Capacity
        {
            get
            {
                ...
                {
#endif
                    return _slots.Length - 1; // subtract 1 for blank node at 0 index
#if !Exclude_No_Hash_Array_Implementation
                }
                else
                {
                    return _noHashArray.Length;
                }
#endif
            }
        }
        ...
        public int NextCapacityIncreaseAtCount
        {
            get
            {
                return _resizeBucketsCountThreshold;
            }
        }

        public bool IsHashing
        {
            get => _noHashArray == null;
        }
        ...
        public int EnsureCapacity(int capacity)
        {
...
#if !Exclude_Check_For_Set_Modifications_In_Enumerator
            _incrementForEverySetModification++;
#endif
            ...
            {
#endif
                currentCapacity = _slots.Length - _count;
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
                currentCapacity = _noHashArray.Length - _count;
            }
#endif
            ...
            int calcedNewBucketsArraySize = (int)(_slots.Length / LoadFactorConst) + 1;
            ...
            if (_buckets.Length < calcedNewBucketsArraySize)
            {
        ...
        private bool CheckForModSizeIncrease()
        {
            if (_bucketsModSize < _buckets.Length)
            {
                ...
                else
                {
                    ...
                    if (_bucketsModSize == size1)
                    {
                        if (size2 <= partLength)
                        {
                            _bucketsModSize = size2;
                            return true;
                        }
                        else
                        {
        ...
        private int GetNewBucketsArraySize()
        {
            ...
            if (_currentIndexIntoBucketsSizeArray >= 0)
            {
                if (_currentIndexIntoBucketsSizeArray + 1 < BucketsSizeArray.Length)
                {
                    newArraySize = BucketsSizeArray[_currentIndexIntoBucketsSizeArray + 1];
                }
                else
                {
                    newArraySize = _buckets.Length;
                }
            }
            else
            {
        ...
        private void IncreaseCapacity(int capacityIncrease = -1)
        {
            ...
            if (IsHashing)
            {
                ...
                else
                {
                    ...
                    oldSlotsArraySize = _slots.Length;
                }
                ...
                Array.Copy(_slots, 0, newSlotsArray, 0, _slots.Length); // check the IL, I think Array.Resize and Array.Copy without the start param calls this, so avoid the overhead by calling directly
        ...
        {
            if (newBucketsArraySize == _buckets.Length)
            {
            ...
            else
            {
                ...
                {
                    _buckets = new int[newBucketsArraySize];
                    _bucketsModSize = newBucketsArraySize;

                    if (_currentIndexIntoBucketsSizeArray >= 0)
                    {
                        _currentIndexIntoBucketsSizeArray++; // when the newBucketsArraySize gets used in the above code, point to the next avaialble size - ??? not sure this is the best place to increment this
                    }
                }
                else
                {
                    Array.Clear(_buckets, 0, _bucketsModSize);
                }

                CalcUsedItemsLoadFactorThreshold();

                int bucketsArrayLength = _buckets.Length;
                ...
                if (_firstBlankAtEndIndex == _count + 1)
                {
                    // this means there aren't any blank nodes
                    for (int i = 1; i < pastNodeIndex; i++)
                    {
                        ref TNode t = ref _slots[i];
            ...
            else
            {
                ...
                if (_firstBlankAtEndIndex == _count + 1)
                {
                    ...
                    for (int i = 1; i < pastNodeIndex; i++)
                    {
                        ...
                        int hashIndex = t.hashOrNextIndexForBlanks % bucketsArrayLength;
                        t.nextIndex = _buckets[hashIndex] | (t.nextIndex & MarkNextIndexBitMask);
        ...
        public void TrimExcess()
        {
            ...
            {
#endif
                if (_slots.Length > _firstBlankAtEndIndex && _firstBlankAtEndIndex > 0)
                {
                    Array.Resize(ref _slots, _firstBlankAtEndIndex);
            ...
            {
                if (_noHashArray != null && _noHashArray.Length > _count && _count > 0)
                {
                    Array.Resize(ref _noHashArray, _count);
                }
            }
#endif
        }
        ...
        public bool Add(in T item)
        {
            ...
            else
            {
                ...
                for (i = 0; i < _count; i++)
                {
                    if (_comparer.Equals(item, _noHashArray[i]))
                    {
                        return false;
                    }
                }

                if (i == _noHashArray.Length)
                {
                    SwitchToHashing();
        ...
        private int AddToHashSetIfNotFound(in T item, int hash, out bool isFound)
        {
            ...
            if (_nextBlankIndex >= _slots.Length)
            {
                ...
                hashIndex = hash % _bucketsModSize;
            }

            int firstIndex = _buckets[hashIndex];
            _buckets[hashIndex] = _nextBlankIndex;

            int addedNodeIndex = _nextBlankIndex;
            ref TNode tBlank = ref _slots[_nextBlankIndex];
            if (_nextBlankIndex >= _firstBlankAtEndIndex)
            {
                // the blank nodes starting at firstBlankAtEndIndex aren't chained
            ...
            if (IsHashing)
            {
                ...
                for (int index = _buckets[hashIndex]; index != NullIndex;)
                {
                    ...
                    if (t.hashOrNextIndexForBlanks == hash && _comparer.Equals(t.item, item))
                    {
                        ...
                        {
                            _buckets[hashIndex] = t.nextIndex;
                        }
                        else
                        {
                            _slots[priorIndex].nextIndex = t.nextIndex;
                        }

                        // add node to blank chain or to the blanks at the end (if possible)
                        if (index == _firstBlankAtEndIndex - 1)
                        {
                            if (_nextBlankIndex == _firstBlankAtEndIndex)
                            {
                                _nextBlankIndex--;
                            }
                            _firstBlankAtEndIndex--;
                        }
                        else
                        {
                            t.hashOrNextIndexForBlanks = _nextBlankIndex;
                            _nextBlankIndex = index;
                        }
            ...
            else
            {
                for (int i = 0; i < _count; i++)
                {
                    if (_comparer.Equals(item, _noHashArray[i]))
                    {
                        ...
                        for (int j = i + 1; j < _count; j++, i++)
                        {
                            _noHashArray[i] = _noHashArray[j];
                        }
                        _count--;
                        return true;
                    }
        ...
        public bool RemoveIf(in T item, Predicate<T> removeIfPredIsTrue)
        {
            ...
            else
            {
                for (int i = 0; i < _count; i++)
                {
                    if (_comparer.Equals(item, _noHashArray[i]))
                    {
                        if (removeIfPredIsTrue.Invoke(_noHashArray[i]))
                        {
                            // remove the item by moving all remaining items to fill over this one - this is probably faster than Array.CopyTo
                            for (int j = i + 1; j < _count; j++, i++)
                            {
        ...
        public ref T FindOrAdd(in T item, out bool isFound)
        {
            ...
            {
#endif
                int addedOrFoundItemIndex = AddToHashSetIfNotFound(in item, (_comparer.GetHashCode(item) & HighBitNotSet), out isFound);
                return ref _slots[addedOrFoundItemIndex].item;
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
        ...
        public ref T Find(in T item, out bool isFound)
        {
            ...
            if (IsHashing)
            {
                ...
                return ref _slots[foundNodeIndex].item;
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
        ...
        public ref T FindAndRemoveIf(in T item, Predicate<T> removeIfPredIsTrue, out bool isFound, out bool isRemoved)
        {
            ...
            if (IsHashing)
            {
                ...
                {
                    isFound = true;
                    ref TNode t = ref _slots[foundNodeIndex];
                    if (removeIfPredIsTrue.Invoke(t.item))
                    {
                        if (priorNodeIndex == NullIndex)
                        {
                            _buckets[bucketsIndex] = t.nextIndex;
                        }
                        else
                        {
                            _slots[priorNodeIndex].nextIndex = t.nextIndex;
                        }

                        // add node to blank chain or to the blanks at the end (if possible)
                        if (foundNodeIndex == _firstBlankAtEndIndex - 1)
                        {
                            if (_nextBlankIndex == _firstBlankAtEndIndex)
                            {
                                _nextBlankIndex--;
                        ...
                        {
                            t.hashOrNextIndexForBlanks = _nextBlankIndex;
                            _nextBlankIndex = foundNodeIndex;
                        }
        ...
        private void UnmarkAllNextIndexValues(int maxNodeIndex)
        {
            ...
            for (int i = 1; i <= maxNodeIndex; i++)
            {
                _slots[i].nextIndex &= MarkNextIndexBitMaskInverted;
            }
        }
        ...
        private void UnmarkAllNextIndexValuesAndRemoveAnyMarkedOrUnmarked(bool removeMarked)
        {
            ...
            int nextIndex;
            int priorIndex;
            int lastNonBlankIndex = _firstBlankAtEndIndex - 1;
            for (int i = 0; i < _buckets.Length; i++)
            {
                ...
                while (index != NullIndex)
                {
                    ...
                    if (removeMarked == isMarked)
                    {
                        ...
                        if (priorIndex == NullIndex)
                        {
                            _buckets[i] = nextIndex;
                        }
                        else
                        {
                            _slots[priorIndex].nextIndex = nextIndex;
                        }
        ...
        public List<ChainLevelAndCount> GetChainLevelsCounts(out double avgNodeVisitsPerChain)
        {
            ...
            if (_buckets != null)
            {
                for (int i = 0; i < _buckets.Length; i++)
                {
                    int index = _buckets[i];
                    if (index != NullIndex)
                    {
                        chainCount++;
                        int itemsInChain = 1;

                        while (_slots[index].nextIndex != NullIndex)
                        {
                            index = _slots[index].nextIndex;
                            itemsInChain++;
                        }
        ...
        public void ReorderChainedNodesToBeAdjacent()
        {
            if (_slots != null)
            {
                TNode[] newSlotsArray = new TNode[_slots.Length];
                ...
                newIndex++;
                _nextBlankIndex = newIndex;
                _firstBlankAtEndIndex = newIndex;
                _slots = newSlotsArray;
            }
        }
        ...
        public bool TryGetValue(T equalValue, out T actualValue)
        {
            ...
            if (IsHashing)
            {
                ...
                if (foundNodeIndex > 0)
                {
                    actualValue = _slots[foundNodeIndex].item;
                    return true;
                }
            ...
            else
            {
                ...
                for (i = 0; i < _count; i++)
                {
                    if (_comparer.Equals(equalValue, _noHashArray[i]))
                    {
                        actualValue = _noHashArray[i];
                        return true;
                    }
                }
        ...
        public void UnionWith(IEnumerable<T> other)
        {
            ...
            if (IsHashing)
            {
                ...
                {
                    AddToHashSetIfNotFound(in item, (_comparer.GetHashCode(item) & HighBitNotSet), out bool isFound);
                }
#if !Exclude_No_Hash_Array_Implementation
            }
            else
        ...
        public void IntersectWith(IEnumerable<T> other)
        {
            ...
            else
            {
                ...
                if (foundItemCount == 0)
                {
                    _count = 0; // this is the equivalent of calling Clear
                }
                else
                {
        ...
        public bool IsProperSubsetOf(IEnumerable<T> other)
        {
            ...
            {
                if (_count == 0 && collection.Count > 0)
                {
            ...
            if (IsHashing)
            {
                ...
                foreach (T item in other)
                {
                    ...
                    if (notFoundAtLeastOne && foundItemCount == _count)
                    {
                ...
                return notFoundAtLeastOne && foundItemCount == _count; // true if all of the items in the set were found in other and at least one item in other was not found in the set
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
        ...
        public bool IsSubsetOf(IEnumerable<T> other)
        {
            ...
            if (other is ICollection<T> collection && _count > collection.Count)
            {
                return false;
            }
            ...
            if (IsHashing)
            {
                ...
                return foundItemCount == _count; // true if all of the items in the set were found in other
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
        ...
        public bool IsProperSupersetOf(IEnumerable<T> other)
        {
            ...
            if (IsHashing)
            {
                ...
                return foundItemCount < _count; // true if all of the items in other were found in set and at least one item in set was not found in other
#if !Exclude_No_Hash_Array_Implementation
            }
            else
            {
        ...
        public bool IsSupersetOf(IEnumerable<T> other)
        {
            ...
            if (IsHashing)
            {
                ...
                foreach (T item in other)
                {
                    if (!FindInSlotsArray(in item, (_comparer.GetHashCode(item) & HighBitNotSet)))
                    {
                        return false;
                    }
                }
        ...
        public bool Overlaps(IEnumerable<T> other)
        {
            ...
            if (other == this)
            {
                return _count > 0; // return false if there are no items when both sets are the same, otherwise return true when both sets are the same
            }
        ...
        public bool SetEquals(IEnumerable<T> other)
        {
            ...
            if (other is ICollection c)
            {
                ...
                if (other is HashSet<T> hset && Equals(hset.Comparer, Comparer))
                {
                    if (hset.Count != _count)
                    {
                        return false;
                    }
                ...
                if (other is FastHashSet<T> fhset && Equals(fhset.Comparer, Comparer))
                {
                    if (fhset.Count != _count)
                    {
                        return false;
                    }
                    ...
                    if (IsHashing)
                    {
                        ...
                        if (fhset.IsHashing)
                        {
                            ...
                            for (int i = 1; i < pastNodeIndex; i++)
                            {
                                ...
                                if (_slots[i].nextIndex != BlankNextIndexIndicator) // skip any blank nodes
                                {
                                    if (!fhset.FindInSlotsArray(in _slots[i].item, _slots[i].hashOrNextIndexForBlanks))
                                    {
                                        return false;
                                    }
                                }
                        ...
                        else
                        {
                            for (int i = 1; i < pastNodeIndex; i++)
                            {
                                if (_slots[i].nextIndex != BlankNextIndexIndicator) // skip any blank nodes
                                {
                                    if (!fhset.FindInNoHashArray(in _slots[i].item))
                                    {
                                        return false;
                                    }
                                }
        ...
        public void SymmetricExceptWith(IEnumerable<T> other)
        {
            ...
            foreach (T item in other)
            {
                addedNodeIndex = AddToHashSetIfNotFoundAndMark(in item, (_comparer.GetHashCode(item) & HighBitNotSet));
                ...
                if (t.hashOrNextIndexForBlanks == hash && _comparer.Equals(t.item, item))
                {
                    ...
                    if ((t.nextIndex & MarkNextIndexBitMask) == 0)
                    {
                        ...
                        else
                        {
                            ...
                            _slots[priorIndex].nextIndex = t.nextIndex | (_slots[priorIndex].nextIndex & MarkNextIndexBitMask);
                        }
        ...
        public int RemoveWhere(Predicate<T> match)
        {
            ...
            if (IsHashing)
            {
                ...
                for (int i = 0; i < _buckets.Length; i++)
                {
                    ...
                    for (int index = _buckets[i]; index != NullIndex;)
                    {
                        ref TNode t = ref _slots[index];
            ...
            else
            {
                ...
                for (i = _count - 1; i >= 0; i--)
                {
                    if (match.Invoke(_noHashArray[i]))
                    {
                        removeCount++;
        ...
        private class FastHashSetEqualityComparer : IEqualityComparer<FastHashSet<T>>
        {
            ...
            public int GetHashCode(FastHashSet<T> obj)
            {
                if (obj == null)
                {
                ...
                else
                {
                    unchecked
                    {
                        ...
                        {
#endif
                            int pastNodeIndex = obj._slots.Length;
                            if (obj._firstBlankAtEndIndex < pastNodeIndex)
                            {
                                pastNodeIndex = obj._firstBlankAtEndIndex;
                            }

                            for (int i = 1; i < pastNodeIndex; i++)
                            {
                                if (obj._slots[i].nextIndex != 0) // nextIndex == 0 indicates a blank/available node
                                {
                                    ...
                                    hashCode += obj._slots[i].hashOrNextIndexForBlanks;
                                }
                            }
#if !Exclude_No_Hash_Array_Implementation
                        }
                        else
                        {
                            ...
                            {
                                // sum up the individual item hash codes - this way it won't matter what order the items are in, the same resulting hash code will be produced
                                hashCode += obj._noHashArray[i].GetHashCode();
                            }
                        }
#endif
                        return hashCode;
        ...
        {
            private readonly FastHashSet<T2> _set;
...
#if !Exclude_Check_For_Set_Modifications_In_Enumerator
            private readonly int _incrementForEverySetModification;
#endif
            ...
            public FastHashSetEnumerator(FastHashSet<T2> set)
            {
                ...
                {
#endif
                    _currentIndex = NullIndex; // 0 is the index before the first possible node (0 is the blank node)
#if !Exclude_No_Hash_Array_Implementation
                }
                else
                {
...
#if !Exclude_Check_For_Set_Modifications_In_Enumerator
                _incrementForEverySetModification = set._incrementForEverySetModification;
#endif
            }
            ...
            public bool MoveNext()
            {
...
#if !Exclude_Check_For_Set_Modifications_In_Enumerator
                if (_incrementForEverySetModification != _set._incrementForEverySetModification)
                {
                ...
                if (_set.IsHashing)
                {
                    ...
                    {
                        _currentIndex++;
                        if (_currentIndex < _set._firstBlankAtEndIndex)
                        {
                            if (_set._slots[_currentIndex].nextIndex != BlankNextIndexIndicator)
                            {
                                return true;
                            }
                        }
                        ...
                        {
                            _currentIndex = _set._firstBlankAtEndIndex;
                            return false;
                        }
                    }
#if !Exclude_No_Hash_Array_Implementation
            ...
            public T2 Current
            {
                get
                {
                    ...
                    if (_set.IsHashing)
                    {
                        ...
                        if (_currentIndex > NullIndex && _currentIndex < _set._firstBlankAtEndIndex)
                        {
                            return _set._slots[_currentIndex].item;
                        }
#if !Exclude_No_Hash_Array_Implementation
                    }
                    else
                    {
                        if (_currentIndex >= 0 && _currentIndex < _set._count)
                        {
                            return _set._noHashArray[_currentIndex];
                        }
                    }
#endif
                    return default;
